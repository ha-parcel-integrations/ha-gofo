# Automatic parcel tracking from e-mail (IMAP → GOFO Express)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the GOFO Express tracking code, and register it with `gofo.track_parcel` — fully automatic, no extra custom component required.

GOFO Express is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `gofo.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ gofo.track_parcel
                                     │
...
- This integration, with the `gofo.track_parcel` action available (field `tracking_code`).
...
The regex matches GOFO's own `GF<CC><digits>` shape, anchored to the six
confirmed country prefixes:

```
(?<![0-9A-Za-z])GF(?:US|CA|IT|FR|ES|NL)[0-9]{6,}
```

The integration itself accepts any non-empty code (see `config_flow.py`'s
`valid_tracking_code`), but matching that broadly against e-mail text would
hit every order number in the mail — so the example anchors on the known
`GF<CC>` prefixes instead. **If GOFO expands to a market not listed here,
add its two-letter code to the alternation**; the AI fallback will surface
unrecognised formats in the meantime.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bGF…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you add a prefix.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@gofoshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: GFUS01011000000001","initial":true,"folder":"INBOX","username":"test"}'
```

Then `gofo.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
