...
Home Assistant custom integration for **GOFO Express** parcel tracking.
...
private `carrier-research/gofo/api/` and are **never** copied here.
...
GOFO is a **country-split code carrier**: `api.py`'s `extract_country()` reads
the two letters right after `GF` in the tracking code (`GF<CC>…`) and picks
one of two mutually exclusive, keyless JSON transports — there is no account,
no postcode, and no separate country selector in the config/options flow. A
code with no recognised `GF<CC>` prefix is never routed to a request at all
(`async_get_parcel` returns `None` without a network call); `config_flow.py`
logs a one-shot warning at add-time so this doesn't fail silently.

- **`waybillNo` is the barcode, never `trackingNumber`.** A real IT capture
  had a `trackingNumber` field completely different from the code the user
  entered (a local-carrier id, `BG-…`) while `waybillNo` matched. Every
  place that used to read a generic `trackingNumber` field (the coordinator's
  not-found/backfill placeholder included) reads `waybillNo` instead —
  `trackingNumber` is redacted from diagnostics, never used as an identifier.
- **Status comes from `processCode`, not the item's own `status` text.**
  `parcels.py`'s `resolve_status()` reads the latest event's `processCode`
...
```
python -m pytest tests/ --cov=custom_components.gofo
```
