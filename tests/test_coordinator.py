"""Tests for the GOFO Express coordinator: fetching, caching and events.
...
from custom_components.gofo.api import GOFOExpressApiError
...
from custom_components.gofo.coordinator import GOFOExpressCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """Same parcel as ``active_sample`` one hop earlier — before "Out for Delivery"."""
    sample = active_sample(code)
    events = sample["trackEventList"][1:]  # drop the "208 Out for Delivery" entry
    sample.update(
        {
            "status": "Transit",
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = GOFOExpressApiError("HTTP 500")
...
async def test_update_backfills_missing_waybill_no(hass):
    """An edge payload without waybillNo keeps the requested code."""
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
    sample = active_sample()
    del sample["waybillNo"]
    client = AsyncMock()
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_sample()
    coordinator = GOFOExpressCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"waybillNo": "GONE"}
