"""Tests for GOFO Express sensor property logic."""
...
from custom_components.gofo.sensor import (
    ...
    GOFOExpressNextDeliverySensor,
    GOFOExpressParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = GOFOExpressIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = GOFOExpressParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = GOFOExpressParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_none_without_moments():
    sensor = GOFOExpressNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = GOFOExpressDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = GOFOExpressLastUpdateSensor(coordinator, _entry())
...
def test_awaiting_pickup_counts_rerouted_parcel_without_pickup_flag():
    ...
    sensor = GOFOExpressAwaitingPickupSensor(_coordinator([rerouted, _parcel("HOME")]), _entry())
    assert sensor.native_value == 1
