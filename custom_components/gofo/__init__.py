"""GOFO Express parcel tracker custom component for Home Assistant."""
...
from .api import GOFOExpressApiClient
...
@dataclass
class GOFOExpressData:
    """Runtime data attached to the GOFO Express config entry."""

    client: GOFOExpressApiClient
    coordinator: GOFOExpressCoordinator


type GOFOExpressConfigEntry = ConfigEntry[GOFOExpressData]


async def async_setup_entry(hass: HomeAssistant, entry: GOFOExpressConfigEntry) -> bool:
    """Set up GOFO Express from a config entry."""
    # No auth: GOFO Express tracking is public, so the HA-managed session is fine.
    client = GOFOExpressApiClient(async_get_clientsession(hass))
    coordinator = GOFOExpressCoordinator(hass, client, entry)
    ...
    entry.runtime_data = GOFOExpressData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: GOFOExpressConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: GOFOExpressConfigEntry) -> bool:
    """Unload the GOFO Express config entry."""
