"""Coordinator for the GOFO Express parcel tracker integration.
...
from .api import GOFOExpressApiClient, GOFOExpressApiError
from .const import (
...
class GOFOExpressCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (GOFOExpressApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, GOFOExpressApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("GOFO Express fetch failed for %s: %s", code, result)
            ...
            # ``waybillNo`` is the code the user entered (api.py already
            # filters both transports' result lists to the item whose
            # waybillNo matches). GOFO's own ``trackingNumber`` field is a
            # *different* local-carrier identifier on transport B — never use
            # it as the barcode. Fall back to the code we asked for so the
            # sensor keeps its key on an edge payload missing waybillNo.
            result.setdefault("waybillNo", code)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "GOFO Express rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("GOFO Express unreachable for all tracked parcels")
