"""Services for the GOFO Express parcel tracker integration.

`gofo.track_parcel` / `gofo.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single GOFO Express hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("GOFO Express is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the GOFO Express services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid GOFO Express tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the GOFO Express services (single-entry integration, so on unload)."""
